...
int RequestHandlerStatic::HandleRequestHelper(
    const Request &req, boost::filesystem::path &complete_path, Response &res) {
  ...
  if (!boost::filesystem::is_regular_file(
          complete_path)) {  // im not sure about the case when we serve
  ...
  if (!uri.extension().string().compare(".txt")) {
  ...
  } else if (!uri.extension().string().compare(".jpg") ||
             !uri.extension().string().compare(".jpeg")) {
    ...
    content_type_header.value = "application/pdf";
             ...
             !uri.extension().string().compare(".htm")) {
    content_type_header.value = "text/html";
  ...
  content_length_header.value =
      std::to_string(boost::filesystem::file_size(complete_path));
  res.headers.push_back(content_length_header);
  return 0;
}
...
void RequestHandlerStatic::HandleRequest(const Request &req, Response &res) {
  ...
  std::ifstream file_stream(complete_path.string(),
