...
bool CrudFileSystemManager::DeleteFile(boost::filesystem::path path) {
  ...
  if (ec == boost::system::errc::success && ListFiles(path.parent_path()).size() == 0) {
...
std::vector<std::string> CrudFileSystemManager::ListFiles(boost::filesystem::path path) {
  std::vector<std::string> file_names;
  if (!boost::filesystem::is_directory(path)) {
    LOG(warning) << path << " is not a directory";
    return file_names;
  }
  LOG(error) << "size = " << file_names.size();
