...
StatusCode CrudHandler::InitHandle(const http::request<http::string_body> req,
                                   http::response<http::dynamic_body>& res) {
  res.version(req.version());
  res.set(boost::beast::http::field::server, "webserver");

  LOG(error) << "Len = " << GetPathLen() << " Entity = " << ParseTarget().first;
...
StatusCode CrudHandler::HandleRequest(const http::request<http::string_body> req,
                                      http::response<http::dynamic_body>& res) {
  StatusCode status_code_ = InitHandle(req, res);
